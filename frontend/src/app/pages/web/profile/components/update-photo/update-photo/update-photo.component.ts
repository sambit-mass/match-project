...
import { CropImageComponent } from '@app/pages/auth/pages/image-upload/components';
import { CustomToastComponent } from '@app/shared/components';
@Component({
  selector: 'app-update-photo',
  standalone: true,
  imports: [CommonModule, TranslatePipe, CustomToastComponent],
  ...
  setAsProfilePhoto(selectedIndex: number, selectedItem: IAllImages) {
    ...
    this.subscriptions.push(
      this._store.dispatch([new UploadProfileImage(formData)]).subscribe({
        next: () => {
          this._store.dispatch(new GetAllImages());
