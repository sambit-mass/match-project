...
@Component({
  selector: 'app-profile-dating-preference',
  standalone: true,
  imports: [
    FormsModule,
    CommonModule,
  ...
  templateUrl: './profile-dating-preference.component.html',
  styleUrl: './profile-dating-preference.component.scss',
})
export class ProfileDatingPreferenceComponent implements OnInit, OnDestroy {
          ...
          if (apiResult[0].length && apiResult[1].length) {
            ...
            const requiredQuestion =
              this.regQuestionList[
                this.regQuestionList.findIndex(item => item.questionnaire_type_id === 7)
              ];
            const cloneQuestion = structuredClone(requiredQuestion);
            const requiredAnswer =
              this.answerList[this.answerList.findIndex(item => item.questionnaire_id === 7)];
  ...
  initializeQuestionFrom() {
    this.questionForm = this.fb.group({
      first_date_idea: this.fb.array(this.optionList['first_date_idea'].map(() => false)),
      date_activity_choice: this.fb.array(this.optionList['date_activity_choice'].map(() => false)),
      important_date_aspect: this.fb.array(
        this.optionList['important_date_aspect'].map(() => false)
      ),
      date_formality_preference: this.fb.array(
        this.optionList['date_formality_preference'].map(() => false)
      ),
      date_attire: this.fb.array(this.optionList['date_attire'].map(() => false)),
    });
  }
