...
@Component({
  selector: 'app-profile-future-aspiration',
  standalone: true,
  imports: [
    FormsModule,
    CommonModule,
  ...
  templateUrl: './profile-future-aspiration.component.html',
  styleUrl: './profile-future-aspiration.component.scss',
})
export class ProfileFutureAspirationComponent {
  max = 243;
  min = 100;
  value1 = 25;
  value2 = 30;
          ...
          if (apiResult[0].length && apiResult[1].length) {
            ...
            const requiredQuestion =
              this.regQuestionList[
                this.regQuestionList.findIndex(item => item.questionnaire_type_id === 9)
              ];
            const cloneQuestion = structuredClone(requiredQuestion);
            const requiredAnswer =
              this.answerList[this.answerList.findIndex(item => item.questionnaire_id === 9)];
