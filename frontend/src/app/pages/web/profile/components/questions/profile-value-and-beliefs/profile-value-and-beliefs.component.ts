...
@Component({
  selector: 'app-profile-value-and-beliefs',
  standalone: true,
  imports: [
    FormsModule,
    CommonModule,
  ...
  templateUrl: './profile-value-and-beliefs.component.html',
  styleUrl: './profile-value-and-beliefs.component.scss',
})
export class ProfileValueAndBeliefsComponent {
          ...
          if (apiResult[0].length && apiResult[1].length) {
            ...
            const requiredQuestion =
              this.regQuestionList[
                this.regQuestionList.findIndex(item => item.questionnaire_type_id === 4)
              ];
            const cloneQuestion = structuredClone(requiredQuestion);
            const requiredAnswer =
              this.answerList[this.answerList.findIndex(item => item.questionnaire_id === 4)];
  ...
  initializeQuestionFrom() {
    this.questionForm = this.fb.group({
      marriage_views: this.fb.array(this.optionList['marriage_views'].map(() => false)),

      important_values: this.fb.array(this.optionList['important_values'].map(() => false)),
      goal_outlook: this.fb.array(this.optionList['goal_outlook'].map(() => false)),
      organization_importance: this.fb.array(
        this.optionList['organization_importance'].map(() => false)
      ),
      vacation_preference: this.fb.array(this.optionList['vacation_preference'].map(() => false)),
      fitness_importance: this.fb.array(this.optionList['fitness_importance'].map(() => false)),
      environmental_concern: this.fb.array(
        this.optionList['environmental_concern'].map(() => false)
      ),
    });
  }
