...
@Component({
  selector: 'app-profile-lifestyle-preference',
  standalone: true,
  imports: [
    FormsModule,
    CommonModule,
  ...
  templateUrl: './profile-lifestyle-preference.component.html',
  styleUrl: './profile-lifestyle-preference.component.scss',
})
export class ProfileLifestylePreferenceComponent implements OnInit, OnDestroy {
          ...
          if (apiResult[0].length && apiResult[1].length) {
            ...
            const requiredQuestion =
              this.regQuestionList[
                this.regQuestionList.findIndex(item => item.questionnaire_type_id === 3)
              ];
            const cloneQuestion = structuredClone(requiredQuestion);
            const requiredAnswer =
              this.answerList[this.answerList.findIndex(item => item.questionnaire_id === 3)];
  ...
  initializeQuestionFrom() {
    this.questionForm = this.fb.group({
      cooking_interest: this.fb.array(this.optionList['cooking_interest'].map(() => false)),
      exercise_frequency: this.fb.array(this.optionList['exercise_frequency'].map(() => false)),
      living_environment: this.fb.array(this.optionList['living_environment'].map(() => false)),
      mealtime_importance: this.fb.array(this.optionList['mealtime_importance'].map(() => false)),
      dietary_preferences: this.fb.array(this.optionList['dietary_preferences'].map(() => false)),
      pet_preference: this.fb.array(this.optionList['pet_preference'].map(() => false)),
      weekend_activities: this.fb.array(this.optionList['weekend_activities'].map(() => false)),
    });
  }
