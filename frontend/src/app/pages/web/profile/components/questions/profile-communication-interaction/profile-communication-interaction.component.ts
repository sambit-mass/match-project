...
@Component({
  selector: 'app-profile-communication-interaction',
  standalone: true,
  imports: [
    FormsModule,
    CommonModule,
  ...
  templateUrl: './profile-communication-interaction.component.html',
  styleUrl: './profile-communication-interaction.component.scss',
})
export class ProfileCommunicationInteractionComponent {
  max = 243;
  min = 100;
  value1 = 25;
  value2 = 30;
          ...
          if (apiResult[0].length && apiResult[1].length) {
            ...
            const requiredQuestion =
              this.regQuestionList[
                this.regQuestionList.findIndex(item => item.questionnaire_type_id === 8)
              ];
            const cloneQuestion = structuredClone(requiredQuestion);
            const requiredAnswer =
              this.answerList[this.answerList.findIndex(item => item.questionnaire_id === 8)];
