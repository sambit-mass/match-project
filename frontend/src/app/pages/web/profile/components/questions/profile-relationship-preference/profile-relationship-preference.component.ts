...
@Component({
  selector: 'app-profile-relationship-preference',
  standalone: true,
  imports: [
    FormsModule,
    CommonModule,
  ...
  templateUrl: './profile-relationship-preference.component.html',
  styleUrl: './profile-relationship-preference.component.scss',
})
export class ProfileRelationshipPreferenceComponent {
          ...
          if (apiResult[0].length && apiResult[1].length) {
            ...
            const requiredQuestion =
              this.regQuestionList[
                this.regQuestionList.findIndex(item => item.questionnaire_type_id === 6)
              ];
            const cloneQuestion = structuredClone(requiredQuestion);
            const requiredAnswer =
              this.answerList[this.answerList.findIndex(item => item.questionnaire_id === 6)];
  ...
  initializeQuestionFrom() {
    this.questionForm = this.fb.group({
      partner_preferences: this.fb.array(this.optionList['partner_preferences'].map(() => false)),
      important_traits_partner: this.fb.array(
        this.optionList['important_traits_partner'].map(() => false)
      ),
      communication_importance: this.fb.array(
        this.optionList['communication_importance'].map(() => false)
      ),
      disagreement_handling: this.fb.array(
        this.optionList['disagreement_handling'].map(() => false)
      ),
      strong_relationship_values: this.fb.array(
        this.optionList['strong_relationship_values'].map(() => false)
      ),
      long_distance_relationship: this.fb.array(
        this.optionList['long_distance_relationship'].map(() => false)
      ),
      future_with_partner: this.fb.array(this.optionList['future_with_partner'].map(() => false)),
    });
  }
