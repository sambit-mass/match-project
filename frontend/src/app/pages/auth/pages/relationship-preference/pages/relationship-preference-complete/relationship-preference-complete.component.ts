...
@Component({
  selector: 'app-relationship-preference-complete',
  standalone: true,
  imports: [AuthHeaderComponent, TranslatePipe, TypingDirective],
  templateUrl: './relationship-preference-complete.component.html',
  styleUrl: './relationship-preference-complete.component.scss',
})
export class RelationshipPreferenceCompleteComponent implements OnDestroy {
  ...
  constructor(
    ...
    this.profileDetails$.subscribe({
      next: details => {
        if (details && details.ques_submit_status !== 6) {
  ...
  goToDatePreferences() {
    ...
    this.subscriptions.push(
      this._store.dispatch(new GetRegQuestions(param)).subscribe({
        next: () => {
          this._router.navigate(['/date-preference']);
        },
        error: apiError => {
