...
@Component({
  selector: 'app-date-preference-complete',
  standalone: true,
  imports: [AuthHeaderComponent, TranslatePipe, TypingDirective],
  templateUrl: './date-preference-complete.component.html',
  styleUrl: './date-preference-complete.component.scss',
})
export class DatePreferenceCompleteComponent implements OnDestroy {
  ...
  constructor(
    ...
    this.profileDetails$.subscribe({
      next: details => {
        if (details && details.ques_submit_status !== 7) {
          this._router.navigate(['/date-preference']);
        }
      },
    });
  }
  goToCommunicationAndInteraction() {
    const param = {
